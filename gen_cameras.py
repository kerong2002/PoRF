...
def gen_cameras(work_dir):
    ...
    image_list = glob(os.path.join(work_dir, 'image/*.png'))
    if len(image_list) < 1:
        image_list = glob(os.path.join(work_dir, 'image/*.jpg'))
    if not len(image_list) < 1:   
        image_list.sort()
    ...
    np.savez(os.path.join(work_dir, 'cameras.npz'), **cam_dict)
    np.savez(os.path.join(work_dir, 'cameras_colmap.npz'), **cam_dict)
    print('Process done!')
