...
class Dataset:
    def __init__(self, conf):
        ...
        filename_db = Path(self.data_dir)/'database.db'
        if not os.path.exists(filename_db):
            print('Error db does not exist!')
            exit()
        connection = sqlite3.connect(filename_db)
        cursor = connection.cursor()
        cursor.execute('SELECT image_id, name FROM new_images')
        fetch = cursor.fetchall()
        self.images_lis = [''] * len(fetch)
        for row in fetch:
            self.images_lis[row[0]] = (os.path.join(self.data_dir, 'image/', row[1]))
        ...
        self.world_mats_np = [camera_dict['world_mat_%d' % idx].astype(np.float32) for idx in range(0, self.n_images)]
        
        # scale_mat: used for coordinate normalization, we assume the scene to render is inside a unit sphere at origin.
        self.scale_mats_np = [camera_dict['scale_mat_%d' % idx].astype(np.float32) for idx in range(0, self.n_images)]
        ...
        gt_world_mats_np = [gt_camera_dict['world_mat_%d' % idx].astype(np.float32) for idx in range(0, self.n_images)]
        self.gt_pose_all = []
